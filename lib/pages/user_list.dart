...
class _UserListState extends State<UserList> {
  ...
  void _filterEmployees(String query) {
    final filteredList = widget.employeeList.where((employee) {
      return employee.firstName.toLowerCase().contains(query.toLowerCase()) ||
                    ...
                    child: ListTile(
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          image: DecorationImage(image: NetworkImage(filteredEmployeeList[index].avatar!.toString()),),
                        ),
                      ),
                      title: Text(
                      ...
                      trailing: IconButton(
                        onPressed: () async {
                          await apiService.deleteData(filteredEmployeeList[index].id.toString(), await tokenService.getAccessToken(),);
                          final refreshedEmployeeList = await apiService.getData(await tokenService.getAccessToken());
                          setState(() {
                            widget.employeeList = refreshedEmployeeList;
                            filteredEmployeeList = refreshedEmployeeList;
                          });
                        },
                        icon: Icon(
                          Icons.delete,
...
class UserSearchDelegate extends SearchDelegate<String> {
  ...
  Widget buildSuggestions(BuildContext context) {
    final suggestions = employeeList
        .where((employee) =>
            employee.firstName.toLowerCase().contains(query.toLowerCase()) ||
            employee.lastName.toLowerCase().contains(query.toLowerCase()) ||
            employee.position.toLowerCase().contains(query.toLowerCase()))
